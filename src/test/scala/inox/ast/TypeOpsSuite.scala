...
//
//    val listD = new AbstractClassDef(listId, Seq(tpD), Seq(nilId, consId), Set())
//    val listT = listD.typed
//
//    val nilD  = new CaseClassDef(nilId, Seq(tpD), Some(listId), Seq(), Set())
//    val nilT  = nilD.typed
//
//    val consD = new CaseClassDef(consId, Seq(tpD), Some(listId), Seq(), Set())
