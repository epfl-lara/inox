...
trait SymbolOps { self: TypeOps =>
  ...
  // Enables simplification of dependent types in positions where they
  // won't influence operational semantics.
  // We have to be careful about chooses and foralls here!
  private[this] val typeSimplifier = Lazy(new SelfTransformer {
    type Env = Set[Variable]

    // function type parameters that shouldn't be simplified as they are
    // used in foralls or chooses
    private val unsimplifiable: Map[Identifier, Set[TypeParameter]] = fixpoint {
      (unsimplifiable: Map[Identifier, Set[TypeParameter]]) =>
        symbols.functions.map { case (id, fd) =>
          id -> (unsimplifiable(id) ++ collect {
            case c: Choose => typeOps.typeParamsOf(c.res.tpe)
            case f: Forall => f.params.flatMap(vd => typeOps.typeParamsOf(vd.tpe)).toSet
            case FunctionInvocation(id, tps, _) =>
              val un = unsimplifiable(id)
              (getFunction(id).typeArgs zip tps)
                .filter(p => un(p._1))
                .flatMap(p => typeOps.typeParamsOf(p._2))
                .toSet
            case ADT(id, tps, _) =>
              getConstructor(id, tps).sort.invariant match {
                case Some(tfd) =>
                  val un = unsimplifiable(tfd.id)
                  (tfd.fd.typeArgs zip tps)
                    .filter(p => un(p._1))
                    .flatMap(p => typeOps.typeParamsOf(p._2))
                    .toSet
                case None =>
                  Set.empty[TypeParameter]
              }
            case _ => Set.empty[TypeParameter]
          } (fd.fullBody))
        }
    } (symbols.functions.keys.map(_ -> Set.empty[TypeParameter]).toMap)

    override def transform(tpe: Type, env: Env): Type = tpe.getType

    override def transform(e: Expr, env: Env): Expr = e match {
      case v: Variable if env(v) => v
      case Choose(res, pred) => Choose(res, transform(pred, env + res.toVariable)).copiedFrom(e)
      case Forall(params, pred) => Forall(params, transform(pred, env ++ params.map(_.toVariable))).copiedFrom(e)
      case FunctionInvocation(id, tps, args) =>
        val un = unsimplifiable(id)
        FunctionInvocation(id, (getFunction(id).typeArgs zip tps).map { case (tp, tpe) =>
          if (un(tp)) tpe else transform(tpe, env)
        }, args map (transform(_, env))).copiedFrom(e)
      case ADT(id, tps, args) =>
        getConstructor(id, tps).sort.invariant match {
          case Some(tfd) =>
            // we rely on the known special shape of invariants here, namely that
            // they have as many type parameters in the same order as the ADT sort
            val un = unsimplifiable(tfd.id)
            ADT(id, (tfd.fd.typeArgs zip tps).map { case (tp, tpe) =>
              if (un(tp)) tpe else transform(tpe, env)
            }, args map (transform(_, env))).copiedFrom(e)
          case None =>
  ...
  def equivalent(e1: Expr, e2: Expr): Boolean = 
    (e1 == e2) || {
      val simp = typeSimplifier.get
      simp.transform(e1, variablesOf(e1)) == simp.transform(e2, variablesOf(e2))
    }
